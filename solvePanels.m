function [gamma, Vs, Cp, NumPan, Gamma, A, b, gamma_dS] = solvePanels(K, L, beta, S, U, betaD)
...
for i = 1:NumPan
    for j = 1:NumPan
        if (i == indx && j == 1) || (i == indx  && j == NumPan) %enforce the Kutta condition at selected panel 
            A(i,j) = 1;
        elseif (i == j && i ~= indx)
...
for i = 1:NumPan
    ...
    Vs(i) = U*sind(betaD(i))-vortex_terms; %calculating surface velocity at ith panel control point
